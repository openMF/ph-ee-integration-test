...
public class BillPayStepDef extends BaseStepDef {
    ...
    public void iCanMockPaymentNotificationRequest() throws JsonProcessingException {
        ...
        jsonBuilder.append("{").append("\"clientCorrelationId\": " + "\"" + BaseStepDef.clientCorrelationId + "\"" + ",")
    ...
    public void iCallThePaymentNotificationApiExpectedStatusOf(int expectedStatus, String callbackurl) throws JSONException {
        ...
        BaseStepDef.response = RestAssured.given(requestSpec).baseUri(billPayConnectorConfig.billPayContactPoint)
                .body(BaseStepDef.inboundTransferReqP2G).expect().spec(new ResponseSpecBuilder().expectStatusCode(expectedStatus).build())
                .when().post(billPayConnectorConfig.paymentsEndpoint).andReturn().asString();
    ...
    public void iCanCallTheBillerRTPRequestAPIWithExpectedStatusOfAndEndpoint(int expectedStatus, String stub)
        ...
        BaseStepDef.response = RestAssured.given(requestSpec).header("Content-Type", "application/json")
                ...
                .header("X-Client-Correlation-ID", clientCorrelationId).header("X-Platform-TenantId", tenant)
                .baseUri(billPayConnectorConfig.billPayContactPoint).body(billRTPReqDTO).expect()
                .spec(new ResponseSpecBuilder().expectStatusCode(expectedStatus).build()).when()

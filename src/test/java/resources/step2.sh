  ...
  if [ $? -eq 0 ]; then
      ...
      kubectl get rolebindings -o custom-columns='KIND:kind,NAMESPACE:metadata.namespace,NAME:metadata.name,SERVICE_ACCOUNTS:subjects[?(@.kind=="ServiceAccount")].name'
      if [ $? -eq 0 ]; then
          echo "done step 2"
      else
          echo "failed step 2"
